...
• Checked SPEC conformance
  └ 1 conforming, 1 unconforming
    internal/foo: nonconforming
      * [Latent] Y doesn't Z in the spec...
      * [New] This branch introduced issue where...
```

(you can see 2 packages above started running the conformance subagent)
...
- Only update the end-user behavior for the check_spec_conformance tool, not other tools.
    - IF we need to change interfaces/apis/abstractions, updating other tools is allowed.
...
- If I have a typo or something in this proposed UI, or inconsisten capitalization, or extra space, don't read into that.
...
• Checked SPEC conformance
  └ 2 conforming, 1 unconforming, 1 error
...
- The subagent is identified somehow. In the example, it was identified by, "Subagent in internal/foo". Does not need to be **uniquely** identified. But unique is not bad.
    - Many possible options here. Ex: "• Checking SPEC conformance in internal/foo"; "• Subagent 1"
- Mid-subagent-run, the last event of the subagent is shown under the subagent.
