...
- We ALSO have the concept of "cas" - content addressible storage. If we spend the LLM tokens to verify that a Go package conforms, we can record that fact.
    - This lets us keep track of which Go packages are known to conform, and which we need to re-check.
    - It also perhaps lets us determine the last known good state of a package, and then inductively determine a changeset still conforms based on diffs to SPEC.md and go code.
        - Ex: "it concorms at merge base, and we only added a single comment, so its conformance must not have changed."
...
- The tools and workflow to keep the CAS up-to-date with spec conformance checks are very manual and unweildy today:
    - run CLI to determine packages that don't have CAS-verified conformance.
    - use package mode on a package one-by-one
        ...
        - Interpret what the agent says:
            - conforms: run CLI command to set CAS on the package for spec conformance
            - doesn't conform: read analysis from agent, filter through my human judgement
            - agent is wrong or nitpicky about irrelevant thing: set CAS conformance anyway
            - agent is right: address the issue (sometimes update spec, sometimes update code, again based on my human judgement)
- We are exploring the "orchestrator" mode to codalotl - it is making many iterative commits, bringing a PR across the finish line.
    - It doesn't currently know about SPEC conformance or CAS at all

Goal:
- Make a new tool, "check_spec_conformance"
- Add tool to orchestrator agent's list of tools
...
- Tool shape: check_spec_conformance(only_changed: true)
    - only_changed: true only checks conformance of packages that have a diff. So i only_changed=true and we're on a git branch, and we modify package internal/foo but not internal/bar, then internal/bar is not checked for conformance.
        - NOTE on diff: we check current state of files on disk vs X, where X is:
            - if we're on the main or master branch, we diff vs HEAD (eg, current commit)
...
- The tool has a presenter that formats the JSON into nice human-digestable text (see review tool).
- does NOT check conformance for pkg if there is no SPEC.md file in the package
- does NOT check the conformance vs any package that cas says already conforms.
...
```json
{
    ...
    "internal/bar": {
        "conforms": false,
        "nonconformances": [
            {"severity": "trivial", "latent": true, "message": "The spec demands X, but intead we have Y"},
...
NOTES on output:
- Allowed severifies: `{"trivial", "minor", "major"}`
- latent=true indicates the non-conformance existed at the merge base. latent=false indicates the diff introduced the nonconformance
    - If there's no diff (ex: branch new feature branch), then everything will be latent=true.

More Details:
- This tool spins up **concurrent** subagents. Each subagent checks one package.
    - Limit concurrency to configurable number (start with 5)
...
- The subagent can just use an existing subagent like "limited_package_mode" for now
- It is given the diff for it's package automatically. It's package definition: all the files in the folder, as well as anything the code unit jail typically allows (ex: data, testdata, etc)
...
- The subagent should be ~read only. It's NOT critical to "lock this down". We can do that later. But the overall intention is this is a read-only operation.
