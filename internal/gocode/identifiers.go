...
//
// It recognizes parenthesized receiver forms such as "(*T).M" and generic receiver forms such as "T[A].M". If ident is not a supported receiver form, ok is false.
// ok only indicates that a structural split was found; receiverExpr may still be syntactically invalid and should be parsed by the caller if validation is required.
