...
// SnippetsByFile returns snippets grouped by file name. If ids is non-empty, only snippets defining at least one requested id will be present (missing ids are ignored),
// and those snippets may also define additional identifiers not in ids. The snippets will be in the same order as they occur on in source.
