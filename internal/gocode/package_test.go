...
func TestPackageAccessorsAndIdentifierHelpers(t *testing.T) {
	pkg, tempDir := newTestPackageFromFiles(t, map[string]string{
		"a.go": dedent(`
			// Package foo documents foo.
			package foo
	...
	assert.False(t, pkg.IsTestPackage())
	require.NotNil(t, pkg.TestPackage)
	assert.True(t, pkg.TestPackage.IsTestPackage())
	assert.Equal(t, tempDir, pkg.AbsolutePath())
	assert.ElementsMatch(t, []string{"a.go", "b_test.go", "gen.go"}, pkg.FileNames())
	assert.ElementsMatch(t, []string{"external_test.go"}, pkg.TestPackage.FileNames())

	ids := pkg.Identifiers(false)
	assert.Contains(t, ids, "Exported")
	assert.Contains(t, ids, "Visible")
	assert.Contains(t, ids, "helper")
	assert.Contains(t, ids, "TestSample")
	assert.Contains(t, ids, "Generated")
	assert.Contains(t, ids, PackageIdentifier)
	assert.Contains(t, ids, PackageIdentifierPerFile("a.go"))
	assert.NotContains(t, ids, "ExternalHelper")
	for _, id := range ids {
		assert.False(t, IsAmbiguousIdentifier(id))
	}

	ambiguousIDs := pkg.Identifiers(true)
	assert.Contains(t, ambiguousIDs, InitIdentifier("a.go", 8, 6))

	aGoIDs := pkg.IdentifiersInFile("a.go", false)
	assert.Contains(t, aGoIDs, "Exported")
	assert.Contains(t, aGoIDs, "Visible")
	assert.Contains(t, aGoIDs, PackageIdentifierPerFile("a.go"))
	assert.NotContains(t, aGoIDs, InitIdentifier("a.go", 8, 6))

	aGoIDs = pkg.IdentifiersInFile("a.go", true)
	assert.Contains(t, aGoIDs, InitIdentifier("a.go", 8, 6))

	nonGenerated, generated := pkg.PartitionGeneratedIdentifiers([]string{"Visible", "Generated", "missing"})
	assert.ElementsMatch(t, []string{"Visible", "missing"}, nonGenerated)
	assert.ElementsMatch(t, []string{"Generated"}, generated)
}

func TestPackageReloadRereadsExistingMainAndTestFiles(t *testing.T) {
	pkg, tempDir := newTestPackageFromFiles(t, map[string]string{
		"a.go": "package foo\n\nfunc Before() {}\n",
		"external_test.go": dedent(`
			package foo_test
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, "a.go"), []byte("package foo\n\nfunc After() {}\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, "external_test.go"), []byte(dedent(`
		package foo_test

		func ExternalAfter() {}
	`)), 0o644))

	reloadedTest, err := pkg.TestPackage.Reload()
	require.NoError(t, err)
	require.NotNil(t, reloadedTest)
	assert.True(t, reloadedTest.IsTestPackage())

	reloadedParent := reloadedTest.parentPackage
	require.NotNil(t, reloadedParent)
	assert.Same(t, reloadedParent, pkg.Module.Packages[pkg.ImportPath])
	assert.NotSame(t, pkg, reloadedParent)
	assert.Nil(t, reloadedParent.GetSnippet("Before"))
	assert.NotNil(t, reloadedParent.GetSnippet("After"))
	assert.Nil(t, reloadedTest.GetSnippet("ExternalBefore"))
	assert.NotNil(t, reloadedTest.GetSnippet("ExternalAfter"))
}

func TestPackageCloneAndDeleteClone(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, clonedPkg)

	cloneModule := clonedPkg.Module
	require.NotNil(t, cloneModule)
	clonePath := cloneModule.AbsolutePath
	assert.True(t, cloneModule.cloned)
	assert.Equal(t, pkg.ImportPath, clonedPkg.ImportPath)
	assert.NotEqual(t, pkg.AbsolutePath(), clonedPkg.AbsolutePath())

	clonedContents, err := os.ReadFile(filepath.Join(clonedPkg.AbsolutePath(), "a.go"))
	require.NoError(t, err)
	assert.Equal(t, "package pkga", string(clonedContents))

	mod.DeleteClone()
	_, err = os.Stat(moduleRoot)
	require.NoError(t, err)

	cloneModule.DeleteClone()
