...
// genRE matches the standard "Code generated … DO NOT EDIT." header at the start of a line. It is used by (*File).IsCodeGenerated to detect generated files, and runs in multiline
// mode so "^" anchors to line starts within the whole file.
var genRE = regexp.MustCompile(`(?m)^//\s*Code generated .* DO NOT EDIT\.?`)

// File represents a Go source file loaded into memory. It stores basic file metadata, the raw contents, and parse artifacts (AST and FileSet). File is intentionally low‑level; higher‑level
// concepts (ex: identifiers and documentation) live on Package. A File is ready to Parse when FileName and Contents are set.
type File struct {
...
// Parse parses f.Contents and sets the AST field and the FileSet. fset can be a token.NewFileSet() used to parse the rest of the package, or a single-use token.NewFileSet() made for
// just this file. If fset is nil, a new one is created and saved to f.FileSet.
...
// PersistContents writes the contents of the File to disk at AbsolutePath, overwriting any existing file. If reparse, a new f.FileSet is created and used to Parse the contents to f.AST;
// otherwise, f.FileSet and f.AST are left unchanged.
//
