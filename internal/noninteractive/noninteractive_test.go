...
func TestFormatAgentFinishedTurnLineIncludesTokens(t *testing.T) {
	...
	line := formatAgentFinishedTurnLine(llmmodel.ModelID("gpt-5.6-sol-high"), llmstream.TokenUsage{
		TotalInputTokens:  10,
		CachedInputTokens: 3,
		TotalOutputTokens: 7,
	})
...
func TestBuildDoneSuccessReport_IdealCachingPrintsActualAndIdealAndDoesNotAffectUsageAndCaching(t *testing.T) {
	...
	withoutIdeal := buildDoneSuccessReport(llmmodel.ModelID("gpt-5.6-sol-high"), actualTurns, completedAssistantTurnsByAgent, actualUsage, false)
	withIdeal := buildDoneSuccessReport(llmmodel.ModelID("gpt-5.6-sol-high"), actualTurns, completedAssistantTurnsByAgent, actualUsage, true)
...
func TestBuildDoneSuccessReport_IdealCachingResetsAcrossAgents(t *testing.T) {
	...
	report := buildDoneSuccessReport(llmmodel.ModelID("gpt-5.6-sol-high"), actualTurns, completedAssistantTurnsByAgent, actualUsage, true)
...
func TestModelReportsCacheWrites(t *testing.T) {
	...
	require.False(t, modelReportsCacheWrites(llmmodel.ModelID("gpt-5.6-sol-high")))
}
...
func (f *recordingFormatter) FormatEvent(e agent.Event, _ int) string {
	...
	case agent.EventTypeToolOutput:
		return "OUT " + e.ToolOutput.Content
	default:
		return ""
	}
}
...
func TestSessionSendUserMessagePrintsPendingToolCallBeforeToolOutput(t *testing.T) {
	originalDelay := toolCallPrintDelay
		...
		sends: []fakeSessionSend{
			{
				events: []agent.Event{
					...
					},
					{
						...
						ToolOutput: agent.ToolOutput{Content: "Applied 1 documentation change"},
					},
					{
						Type:  agent.EventTypeToolComplete,
						Agent: agent.AgentMeta{ID: "root", Depth: 0},
	...
	output := buf.String()
	require.Contains(t, output, "CALL codalotl_cli\n")
	require.Contains(t, output, "OUT Need docs for 3 identifiers\n")
	require.Contains(t, output, "OUT Applied 1 documentation change\n")
...
func TestSessionSendUserMessageJSONToolOutputEmitsStableObject(t *testing.T) {
				...
				events: []agent.Event{
					...
					},
					{
						...
						ToolResult: &llmstream.ToolResult{
							...
							Name:    "ignored_result_name",
							Type:    "function_call",
							Result:  `{"stdout":"Need docs for 3 identifiers\n"}`,
							IsError: false,
						},
					},
					{
	...
	require.NoError(t, json.Unmarshal(lines[3], &toolOutput))
	require.Equal(t, map[string]any{
		"type":    "tool_output",
	...
	var toolComplete jsonToolCompleteEvent
	require.NoError(t, json.Unmarshal(lines[4], &toolComplete))
	require.Equal(t, "tool_complete", toolComplete.Type)
	require.Equal(t, jsonResult{
		Output:  `{"stdout":"Need docs for 3 identifiers\n"}`,
		IsError: false,
	}, toolComplete.Result)
}
...
func TestSessionSendUserMessageSuppressesDescendantFinalAssistantTextOutsideLabeledHumanReadableOutput(t *testing.T) {
	originalDelay := toolCallPrintDelay
	toolCallPrintDelay = 0
	t.Cleanup(func() {
		toolCallPrintDelay = originalDelay
...
func TestSessionSendUserMessageUsesLabeledSubagentLifecycleInHumanReadableOutput(t *testing.T) {
	originalDelay := toolCallPrintDelay
	toolCallPrintDelay = 0
	t.Cleanup(func() {
		toolCallPrintDelay = originalDelay
		...
		sends: []fakeSessionSend{
			{
				events: []agent.Event{
					...
					},
					{
						...
						Agent:                   childAgent,
						AssistantTextFinalizing: true,
						TextContent:             llmstream.TextContent{Content: "does not conform"},
					},
					{
						Type:  agent.EventTypeDoneSuccess,
						Agent: childAgent,
	...
	require.Contains(t, output, "TEXT internal/foo: does not conform\n")
	require.Contains(t, output, "DONE review\n")
	require.NotContains(t, output, "TEXT looked at files\n")
	require.NotContains(t, output, "CALL clarify_public_api\n")
	require.NotContains(t, output, "TEXT checked docs\n")
	require.NotContains(t, output, "nested worker")
}
...
func TestSessionSendUserMessageFormatsDescendantFinalAssistantTextInHumanReadableOutput(t *testing.T) {
	...
	require.Contains(t, output, "TEXT review worker: Verdict: approve\nNo actionable findings.\n")
	require.NotContains(t, output, `TEXT {"decision":"approve"}`)
}

func TestSessionSendUserMessageFallsBackToFinishedForLabeledSubagentWithoutVisibleFinalText(t *testing.T) {
	originalDelay := toolCallPrintDelay
	toolCallPrintDelay = 0
	t.Cleanup(func() {
		toolCallPrintDelay = originalDelay
