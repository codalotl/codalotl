...
type finalMessageStubPresenter struct {
	...
	calls []finalMessagePresenterCall
}

type finalMessagePresenterCall struct {
	call         llmstream.ToolCall
	label        string
	finalMessage string
}

func (p *finalMessageStubPresenter) SubagentFinalMessage(call llmstream.ToolCall, label string, finalMessage string) llmstream.Block {
	p.calls = append(p.calls, finalMessagePresenterCall{
		call:         call,
		label:        label,
...
func TestCustomizeOnlyFinalDescendantAssistantTextUnderToolScope(t *testing.T) {
	m := newModel(colorPalette{}, noopFormatter{}, nil, sessionConfig{}, nil, nil, nil, nil)

	root := agent.AgentMeta{ID: "root"}
	child := agent.AgentMeta{ID: "review-subagent", Depth: 1, Parent: root.ID}
	reviewPresenter := &finalMessageStubPresenter{
	...
	reviewTool := newNamedToolWithPresenter("review", reviewPresenter)
	...
	require.Empty(t, reviewPresenter.calls)
	...
	require.Len(t, reviewPresenter.calls, 1)
	require.Equal(t, *reviewCall, reviewPresenter.calls[0].call)
	require.Equal(t, "review worker", reviewPresenter.calls[0].label)
	require.Equal(t, `{"verdict":"pass"}`, reviewPresenter.calls[0].finalMessage)

	require.Len(t, m.messages, 4)
	require.Equal(t, agent.EventTypeToolCall, m.messages[0].event.Type)
	require.Equal(t, agent.EventTypeAssistantText, m.messages[1].event.Type)
	require.Equal(t, "Checking the diff before the structured result.", m.messages[1].event.TextContent.Content)
	require.Equal(t, agent.EventTypeAssistantText, m.messages[2].event.Type)
	require.Equal(t, "Verdict: pass\nNo actionable findings.", m.messages[2].event.TextContent.Content)
	require.Equal(t, agent.EventTypeToolComplete, m.messages[3].event.Type)
}

func TestAssistantTurnCompleteDoesNotCustomizeDescendantAssistantText(t *testing.T) {
	m := newModel(colorPalette{}, noopFormatter{}, nil, sessionConfig{}, nil, nil, nil, nil)
	...
	require.Equal(t, agent.EventTypeToolCall, m.messages[0].event.Type)
	require.Equal(t, agent.EventTypeAssistantText, m.messages[1].event.Type)
	require.Equal(t, `{"verdict":"pass"}`, m.messages[1].event.TextContent.Content)
}

func TestFinalDescendantAssistantTextWithoutPresenterIsLiteral(t *testing.T) {
	m := newModel(colorPalette{}, noopFormatter{}, nil, sessionConfig{}, nil, nil, nil, nil)
...
func TestFinalDescendantAssistantTextCustomizationDoesNotRequireSuccessfulTerminalEvent(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Len(t, reviewPresenter.calls, 1)
			require.Equal(t, "final reply", reviewPresenter.calls[0].finalMessage)
