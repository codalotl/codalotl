...
- For `SubagentEventPolicySummarizeBySubagent`, the TUI synthesizes one visible child row per started subagent under the parent tool call, using `StartSubagent.Label` as the identifier.
    - While active, the subagent row shows the latest visible descendant event for that subagent.
    - When done, that row shows the final result or error for that subagent.
- `EventTypeStartSubagent` is metadata and is not displayed directly as its own message.
