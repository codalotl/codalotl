...
- Tools have a Call and a Result.
    ...
    - Exception: tools whose presenter uses `Append`, plus the legacy SubAgent calls `update_usage`, `clarify_public_api`, `implement`, and `review`, should NOT replace the Call with Result (they print both).
