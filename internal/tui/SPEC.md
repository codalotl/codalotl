...
- Displayed descendant subagent final messages are driven by descendant `agent.EventTypeAssistantText` events with `AssistantTextFinal=true`.
...
- When a tool presenter does not implement that interface, descendant final assistant-text events are displayed as plain text.
- Non-final descendant assistant-text events are always displayed literally.
