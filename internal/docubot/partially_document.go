...
// errSomeSnippetsFailed indicates that at least one generated documentation snippet could not be applied. It is non-fatal; callers should inspect the returned package
// and updated file set for any successful changes.
var errSomeSnippetsFailed = fmt.Errorf("some snippets failed")

// successfulSnippets returns the snippets that were successfully applied. A snippet is still considered successful if it was partially applied. Future improvement:
// if a snippet is fully rejected, it should not be considered successful.
...
// sliceToSet returns a new set containing all elements of arr, represented as map[string]struct{}. Duplicates in arr are collapsed; a nil or empty arr yields an
// empty, non-nil map.
...
//
// The output begins with "Document these identifiers:" followed by one "- " line per identifier in the order provided by targetIdentifiers. If an identifier equals
// gocode.PackageIdentifier, the line targets the package doc as "package <pkg.Name> (overall package documentation)". If targetFields[id] is non-empty, the line
// notes the fields to document for that identifier.
